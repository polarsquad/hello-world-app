...
const render = (view, ctx = {}) =>
  template(readFileSync(resolve(__dirname, `views/${view}.html`)))(ctx);

function getConfigMaps() {
  const filteredEnvs = Object.keys(process.env)
    .filter((key) => key.includes("configmap"))
    .reduce(
      (obj, key) => ({
        ...obj,
        [key]: { key, value: process.env[key] },
      }),
      {}
    );
  return filteredEnvs;
...
export default (storage) => {
  ...
  router.get("/", (_, res) => {
    Promise.all([storage.get("thing"), storage.get("heroName")]).then(
      ([thing, heroName]) => {
        const data = sample(IMAGES);
        data.storageName = storage.constructor.name;
        data.heroName = heroName;
        data.heroTitle = heroName
          ? `Oh, you like ${heroName}, are you sure?`
